...
const DataRekapitulasi = () => {
  ...
  const [groupBy, setGroupBy] = useState('all'); // 'all' or 'matkul'
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/admin/rekapitulasi?group=${groupBy}`, {
        headers: {
      ...
      setRekapitulasiData(result.data || []);
      setLoading(false);
    } catch (error) {
