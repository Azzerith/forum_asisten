...
const RekapitulasiPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch presensi data for current user only
        const presensiResponse = await fetch(`http://localhost:8080/api/presensi?asisten_id=${user.id}`, {
          headers: {
        ...
        // Fetch rekapitulasi data for current user only
        const rekapitulasiResponse = await fetch(`http://localhost:8080/api/rekapitulasi?asisten_id=${user.id}`, {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
        });
  ...
  // Group attendance data by mata kuliah (filtered for current user)
        ...
        >
          ...
          {rekapitulasiData.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500">
              Tidak ada data rekapitulasi honor yang ditemukan
            </div>
          )}
        </motion.div>
